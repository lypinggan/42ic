...
from iapp.models import User,Options
from iapp.helpers import render_template
from iapp.extensions import db, mail, cache
from iapp.utils.online import login_session, guest_session
...
def configure_template_filters(app):
    ...
    def minitohtml(value):
        return helpers.minitohtml(value)

    @app.template_filter()
    def cssfile(value):
        return app.config['STATIC_DOMAIN'] + 'css/' + value
    @app.template_filter()
    def jsfile(value):
        return app.config['STATIC_DOMAIN'] + 'js/' + value
    @app.template_filter()
    def imagefile(value):
        return app.config['STATIC_DOMAIN'] + 'images/' + value
    @app.template_filter()
    def avatarfile(value):
        return app.config['STATIC_DOMAIN'] + 'avatar/' + value
